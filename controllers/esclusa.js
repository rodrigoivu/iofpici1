...
const Esclusa = require('../models/Esclusa');
...
const registraItem = async( req, res = response ) => {
    ...
    const item = new Esclusa( req.body );
...
const actualizaItem = async( req, res = response ) => {
    ...
    try {
        ...
        const itemFound = await Esclusa.findById( itemId );
        ...
        const item = await Esclusa.findByIdAndUpdate( itemId, params, { new: true });
...
const itemsTodos = async( req, res = response ) => {
    ...
    try {
		...
		const items = await Esclusa.find()
                            	  .sort([['nombre', 1]]);
...
const deleteItem = async( req, res = response ) => {
    ...
    try {
        ...
        const item = await Esclusa.findById( itemId );
        ...
        await Esclusa.findByIdAndDelete( itemId );
