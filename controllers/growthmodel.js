...
const registraItem = async( req, res = response ) => {
    ...
    const item = new Growthmodel( req.body );
...
const actualizaItem = async( req, res = response ) => {
    ...
    try {
        ...
        const itemFound = await Growthmodel.findById( itemId );
        ...
        const item = await Growthmodel.findByIdAndUpdate( itemId, params, { new: true });
...
const itemsTodos = async( req, res = response ) => {
    ...
    try {
		...
		const items = await Growthmodel.find()
                            	       .sort([['nombre', 1]]);
...
const deleteItem = async( req, res = response ) => {
    ...
    try {
        ...
        const item = await Growthmodel.findById( itemId );
        ...
        await Growthmodel.findByIdAndDelete( itemId );
